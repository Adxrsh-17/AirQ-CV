...
def main():
    ...
    # Target is anomaly from climatology
    train_df['no2_anomaly'] = train_df['no2_target'] - train_df['no2_clim_mean']
    test_df['no2_anomaly'] = test_df['no2_target'] - test_df['no2_clim_mean']
    
    features = ['x_norm', 'y_norm', 'sin_doy', 'cos_doy', 'ndvi', 'ndbi', 'ndmi']
    
    X_train = train_df[features]
    y_train = train_df['no2_anomaly']
    
    X_test = test_df[features]
    y_test_anomaly = test_df['no2_anomaly']
    y_test_true = test_df['no2_target']
    y_test_clim = test_df['no2_clim_mean']
    
    print("Training LightGBM Spatial Regressor for NO2...")
    ...
    print("Saving model...")
    model.booster_.save_model(models_dir / 'no2_spatial_lgbm.txt')
    ...
    print(f"=== NO2 Long-Range (150-Day) Performance ===")
    print(f"Climatology Baseline R² : {r2_clim:.4f}")
    print(f"Climatology Baseline RMSE: {rmse_clim:.4e}")
    print(f"Spatial RF (LGBM) R²    : {r2_model:.4f}")
    print(f"Spatial RF (LGBM) RMSE  : {rmse_model:.4e}")
